<template>
  <div class="channel app-container">
      ...
      <template>
        <div>
            ...
            <el-form-item label="逾期天数" style="width: 60%">
              <el-col :span="11">
                <el-form-item prop="overDaysMin">
                  <el-input :class="$style['datetime']" placeholder v-model="form.overDaysMin"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="1" style="margin-left: 3px;">&nbsp;-</el-col>
              <el-col :span="11">
                <el-form-item prop="overDaysMax">
                  <el-input :class="$style['datetime']" placeholder v-model="form.overDaysMax"></el-input>
                </el-form-item>
              </el-col>
            </el-form-item>
            ...
            <el-form-item label="待赔付金额" style="width: 60%">
              <el-col :span="11">
                <el-form-item prop="sPaidAmountMin">
                  <el-input :class="$style['datetime']" placeholder v-model="form.sPaidAmountMin"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="1" style="margin-left: 3px;">&nbsp;-</el-col>
              <el-col :span="11">
                <el-form-item prop="sPaidAmountMax">
                  <el-input :class="$style['datetime']" placeholder v-model="form.sPaidAmountMax"></el-input>
                </el-form-item>
              </el-col>
            </el-form-item>
          </el-form>
          ...
          <div :class="$style['search-btn-box']">
            ...
            >查询</el-button>
            <el-button
              :loading="loadingBtn.export"
              @click="exportExcelConfirm"
            ...
            >导出</el-button>
            <el-button
              :loading="loadingBtn.declaration"
            ...
            >批量申报</el-button>
            <el-button
              :loading="loadingBtn.compensation"
              @click="compensationConfirm"
              class="iconfont icon-piliang-copy"
              size="medium"
              style="font-size: 14px;"
              type="primary"
    ...
    <div :class="$style['content']">
      <blockTitle :hide="trueVal">担保代偿列表</blockTitle>
      <div :class="$style['table-content']">
        <el-table
          ...
          v-loading="loading"
        >
          <el-table-column :reserve-selection="trueVal" label="借据号" prop="LISTID" type="selection"></el-table-column>
          <el-table-column label="借据号" prop="LISTID" min-width="238px"></el-table-column>
          <el-table-column label="渠道名称" prop="CHANNELNAME"></el-table-column>
          <el-table-column label="产品名称" prop="PRODNAME"></el-table-column>
          <el-table-column label="姓名" prop="CUSTNAME"></el-table-column>
          <el-table-column label="证件号码" min-width="165px" prop="IDNO"></el-table-column>
          <el-table-column label="借款金额" prop="LOANAMOUNT"></el-table-column>
          <el-table-column label="借款期限" prop="LOANTERM"></el-table-column>
          <el-table-column label="放款日期" min-width="100px" prop="BEGINDATE"></el-table-column>
          <el-table-column label="担保费率" prop="GUARFEERATE"></el-table-column>
          <el-table-column label="借款费率（年）" prop="INTERESTRATE"></el-table-column>
          <el-table-column label="应收担保费" prop="SASSUREAMOUNT"></el-table-column>
          <el-table-column label="应付通道费" prop="SCHANNELAMOUNT"></el-table-column>
          <el-table-column label="实收通道费" prop="RCHANNELAMOUNT"></el-table-column>
          <el-table-column label="实收担保费" prop="RASSUREAMOUNT"></el-table-column>
          <el-table-column label="应还日期" min-width="100px" prop="SDATE"></el-table-column>
          <el-table-column label="逾期天数" prop="OVERDAYS"></el-table-column>
          <el-table-column label="逾期利息" prop="BINTE"></el-table-column>
          <el-table-column label="实还总金额" prop="RAMOUNT"></el-table-column>
          <el-table-column label="实还本金" prop="RCAPI"></el-table-column>
          <el-table-column label="实还利息" prop="RINTE"></el-table-column>
          <el-table-column label="实还罚息" prop="RFINE"></el-table-column>
          <el-table-column label="实还违约金" prop="RFOUL"></el-table-column>
          <el-table-column label="贷款余额" prop="BAL"></el-table-column>
          <el-table-column label="待赔付金额" prop="SPAIDAMOUNT"></el-table-column>
          <el-table-column label="应收理赔本金" prop="SSETTLEBAL"></el-table-column>
          <el-table-column label="应收理赔利息" prop="SSETTLEINTE"></el-table-column>
          <el-table-column label="实收理赔本金" prop="RSETTLEBAL"></el-table-column>
          <el-table-column label="实收理赔利息" prop="RSETTLEINTE"></el-table-column>
          <el-table-column :formatter="formatAcFlag4Detail" label="借据状态" prop="ACFLAG"></el-table-column>
          <el-table-column :formatter="formatAssureStatus" label="担保状态" prop="ASSURESTATUS"></el-table-column>
          <el-table-column label="申报日期" min-width="100px" prop="DECLAREDATE"></el-table-column>
          <el-table-column label="代偿日期" min-width="100px" prop="COMPENSATEDATE"></el-table-column>
          <el-table-column label="操作" min-width="210px" prop>
            <template slot-scope="scope">
              <el-button @click="detail(scope.row)" size="small" type="primary">查询</el-button>
              <el-button
                @click="declarationOneConfirm(scope.row)"
                size="small"
                type="primary"
                v-if="scope.row.ASSURESTATUS == '0'"
              >申报</el-button>
              <el-button
                @click="compensationOneConfirm(scope.row)"
                size="small"
                type="primary"
                v-if="scope.row.ASSURESTATUS == '2'"
              >代偿</el-button>
            </template>
          </el-table-column>
        </el-table>
...
interface Iform {
  ...
  compensateDate: string; // 代偿日期
  channelNo: string; // 渠道名称
  overDaysMin: string; // 逾期天数范围下限
  overDaysMax: string; // 逾期天数范围上限
  sPaidAmountMin: string; // 待赔付金额范围下限
  sPaidAmountMax: string; // 待赔付金额范围上限
  loanDateMin: string; // 放款日期范围下限
  loanDateMax: string; // 放款日期范围上限
  assureStatus: string; // 担保状态
}
...
interface IloadingBtn {
  search: boolean; // 搜索
  export: boolean; // 导出
  declaration: boolean; //申报状态
  compensation: boolean; // 代偿状态
  declarationOne: boolean; // 申报一个
  compensationOne: boolean; // 代偿一个
}
...
export default class extends Vue {
    ...
    overDaysMin: [
      {
        required: false,
        pattern:
          "(?:^[-]?[1-9]\\d{0,4}(?:$|(?:,\\d{3})*(?:$|(\\.\\d{1,2}$))))|(?:(?:^[0](\\.\\d{1,2})?)|(?:^[-][0]\\.\\d{1,2}))$",
        message: "请输入正确的天数",
        trigger: "blur"
      }
    ],
    ...
    this.$refs["form"].validate(valid => {
      if (valid) {
        ...
        this.$refs &&
          ...
          this.$refs.singleTable.clearSelection(); // 清除选中元素
        this.getData();
      } else {
        return false;
      }
  ...
  async exportExcel(list: object) {
    ...
    import("@/vendor/Export2Excel").then(async excel => {
      ...
      let data = await this.$api.grtCps.exportExcel({
        ...this.form,
        listIds: list
      });
      data = data.data.data;
  ...
  async compensationConfirm() {
    ...
    try {
      if (res.data.resultCode === "0000") {
        ...
        this.$confirm(
          `共选中${ids.length}条数据,剩余担保费${leftGuarFee},待赔付金额${leftsAmt},是否确认代偿?`,
          "提示",
          {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
  ...
  detail(row: any) {
    this.$router.push({
      path: "/guaranteeM/detail",
      query: { listId: row.LISTID, channelId: row.CHANNELID }
    });
  }

  declarationOneConfirm(row: any) {
  ...
  async compensationOneConfirm(row: any) {
  ...
  getIds() {
    ...
    let list = this.$refs.singleTable.store.states.selection; // 选中的元素
    let ids: string[] = [];
    list.forEach((item: { LISTID: string }) => {
      ids.push(item.LISTID);
    });
    return ids;
  }
  formatAssureStatus(row: any, column: any, cellValue: any) {
